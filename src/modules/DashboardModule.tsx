...
import { Product, StockAlert, ReturnTicket, AppUser, InventoryMovement } from '../types';
import { RealTimeFlowChart } from '../components/dashboard/RealTimeFlowChart';
import { TopDemandChart } from '../components/dashboard/TopDemandChart';
  ...
  return (
    <div className="space-y-6 animate-fade pb-10">
        ...
        <div className="lg:col-span-4 grid grid-cols-2 lg:grid-cols-1 lg:grid-rows-2 gap-4 lg:gap-6">
          ...
          <div className="bg-blue-600 rounded-[1.5rem] lg:rounded-[2.5rem] p-5 lg:p-8 flex flex-col justify-center shadow-lg shadow-blue-600/20 text-white relative overflow-hidden">
             ...
             <h3 className="text-3xl lg:text-5xl font-black leading-none tracking-tighter">
               {movements.filter(m => new Date(m.createdAt || (m as any).creadoEn).toDateString() === new Date().toDateString()).length}
             </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Análisis de Flujo */}
        <RealTimeFlowChart movements={movements} period={period} setPeriod={setPeriod} />

        {/* Top Demanda */}
        <TopDemandChart movements={movements} period={period} />
